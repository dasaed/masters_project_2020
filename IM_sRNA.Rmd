...
Some OPTIONAL memory cleaning (useful if you want to run this from R Studio and with a clean environment)
```{r}
...
The following block of code is copied from the h2o documentation site, as it's their recommended way for downloading and installing h2o for R. Here is the [link](http://h2o-release.s3.amazonaws.com/h2o/rel-zahradnik/4/index.html) for those interested in it.
...
# the scale function can scale and center the data for us, but we would still need the training data's standard deviation and means to be able to apply the same normalization to our testing data sets. 
...
# ..10.2 Compare OrigRF vs OrigRF Scaled Model ----
# By looking at the difference between the number of predictions the Orig. RF and the other RF,
...
# ..10.3 Compare OrigRF vs OrigRF Norm Model ----
...
# ..10.4 Compare OrigRF vs H2O Model ----
...
# ..10.5 Compare OrigRF vs H2O Model Scaled ----
...
# ..10.6 Compare OrigRF vs H2O Model Normalized ----
...
# ..11.2 Compare OrigRF vs OrigRF Scaled Model ----
# By looking at the difference between the number of predictions the Orig. RF and the other RF,
# we can conclude that the models are similar enough in accuracy
...
# ..11.3 Compare OrigRF vs OrigRF Norm Model ----
...
# ..11.4 Compare OrigRF vs H2O Model ----
...
# ..11.5 Compare OrigRF vs H2O Model Scaled ----
...
# ..11.6 Compare OrigRF vs H2O Model Normalized ----
...
plot(origRF_slt2_performance$acc, col = "blue", lwd = 5 ) 
...
lines(as.double(unlist(origRF_scaled_slt2_performance$acc@x.values)), as.double(unlist(origRF_scaled_slt2_performance$acc@y.values)), col = "black", lwd = 1) 
...
plot(origRF_lu_performance$acc, col = "blue", lwd = 5)
...
lines(as.double(unlist(origRF_scaled_lu_performance$acc@x.values)), as.double(unlist(origRF_scaled_lu_performance$acc@y.values)), col = "black", lwd = 1) 
...
plot(origRF_slt2_performance$PR,  col = "blue", lwd = 5 )
...
plot(origRF_lu_performance$PR,  col = "blue", lwd = 5 )
...
plot(origRF_slt2_performance$SS, col = "blue", lwd = 5)
...
plot(origRF_lu_performance$SS, col = "blue", lwd = 5)
...
plot(h2o.F1(rfh2o_slt2_performance), col = "blue")
...
# NOTE: many of these settings were obtained through trial and error, and/or based on the suggestions provided by LIME and its libraries. Since LIME's results are not always the same, we used the settings that to our opinion generated the most consistent and valid explanations. Of course, we encourage others to try out other settings and experinment further.
# * Settings for "explainer"
#     bin_continuous = TRUE --> Makes explanation easier, and having this setting as true helped in obtaining more consistant and reliable results.
...
#           Side Note: We tried a couple of runs of the script with euclidean distance, but it didn't seem to have much of an impact. Additionally, the different types of settings, such as different gower powers, kernel widths, and other distance functions, that can be tweaked in LIME means that a 100% thorough analysis of every possibility may be nearly impossible. For this reason, we tried to stick to the default settings as offered by LIME. 
...
## Data to Explain
# sampleData, sampleData_scaled, and sampleData_norm all contain the same feature, and hence, we should be getting similar explanations in all of our results. We also repeat the same sample 4 times, with purpose of seeing how LIME behaves and making sure LIME's explanations are consistent and valid. 
...
# 15. LIME with RandomForest Library ----
# ..15.1 In original Model as is ----
...
lime_explanations_orig <- explain(as.data.frame(sampleData[,c(1:7)]), lime_explainer_orig, n_labels = 1, n_features = 7, n_permutations = 2000)

plot1 <- plot_features(lime_explanations_orig)
plot1

# ..15.2 In original scaled Model ----
...
lime_explanations_orig_scaled <- explain(as.data.frame(sampleData_scaled[,c(1:7)]), 
                                         lime_explainer_orig_scaled, n_labels = 1, 
                                         n_features = 7, n_permutations = 2000
                                         )

plot2 <- plot_features(lime_explanations_orig_scaled)
plot2
  
# ..15.3 In original normalized Model ----

predict(origRF_scaled, sampleData_scaled[c(1,5),-c(8:9)], type = "prob")
lime_explanations_orig_scaled <- explain(as.data.frame(sampleData_scaled[,c(1:7)]), 
                                         lime_explainer_orig_scaled, n_labels = 1, 
                                         n_features = 7, n_permutations = 2000, 
                                         #dist_fun = "euclidean", kernel_width = 5
                                         dist_fun = "gower")
plot_features(lime_explanations_orig_scaled)

# 16. LIME in Orig RF Conclusions ---- 
#  We tried applying LIME to the Orig RF(in part 15) hoping it would work like a plug-and-play application. To our surprise, you can't get LIME to properly work with the "Original" RF model because you must normalize, or scale the data. Since LIME randomizes the permutation points, based on our experinments it seems that LIME is using its own randomization algorith (ie. we couldn't set a seed with the standard R libraries), and hence, the exact results we obtained in this area may not be completely repeatable. However, for the particular case in part 15, that may be irrelevant as the point we are trying to prove here is that LIME's explanations when the data is not scaled or normalized are inconsistant, and hence cannot be trusted.  The top 4 plots should be identical to each other, and the bottom 4 plots should also be identical to each other. However, there's usually at least 1 plot that ranks the feature contributions differently, or that may have a feature "Supporting" in one plot, but "Contradicting" in the next. Due to LIME's nature, obtaining slightly different results was expected, but not to a degree that would make the explanation invalid.  
...
# 17. LIME for the H2O RF models  ----
# Initial struggles with getting LIME set up with the original random forest lead us to believe that since RandomForest wasn't a support method by default in the Lime Library, that maybe using a supported method would help us overcome these problems. During our investigation, we discovered that H2O offered additional IM methods (PDPs and SHAP), and that their algorithms were natively supported by the Lime library. Thinking we could kill 3 birds (test 3 IMs) with one stone(one package), we proceeded to create the equivalent proxy model in H2O and run tests with it. The H2O models were actually created prior to our normalized and scaled data tests, as we hadn't realized LIMEs problems with our data at the time. 
...
lime_explanations_rfh2o <- explain( as.data.frame(sampleData[,c(1:7)] ),lime_explainer_rfh2o, 
                                    n_labels = 1,  n_features = 7, 
                                    n_permutations = 2000 )
plot_features(lime_explanations_rfh2o) 


# ..17.2 In H2O RF scaled Model ----
...
# ..17.3 LIME to H2O RF normalized Model ----
...
# ..17.4 Applying LIME to GLM Model (BONUS) ----
...
# LIME CONCLUSIONS ----
#
# A known problem with LIME is that explanations may be inconsistant from one instance to the next, even when the instances are very similar to each. With this in mind, each instance we tested was tested at least 4 times (we manually ran this section over and over again) , and with a high enough number of permutations, we managed to get somewhat consistent results. However, some of the results obtained from  LIME were heavily flawed as sometimes it would yield explanations that contradicted the result. 
