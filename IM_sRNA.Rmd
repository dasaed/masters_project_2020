...
write.csv(slt2_test_samples, "./DataSets/slt2_data_to_analyze.csv", row.names = TRUE, col.names = TRUE)

write.csv(lu_test_samples,   "./DataSets/lu_data_to_analyze.csv", row.names = TRUE, col.names = TRUE)

lime_explainer_orig_norm <- lime(as.data.frame(trainData_norm[, c(1:7)]), origRF_norm,
                                   bin_continuous = TRUE, quantile_bins=FALSE, 
                                   use_density = TRUE, n_bins = 10)

pdf("./Results/lime_slt2_explanations.pdf")
for( i in 1:nrow(origRF_norm_slt2_pred_ims) ){
  temp_df <- data.frame(origRF_norm_slt2_pred_ims[i,])
  temp_df <- temp_df[ rep(seq_len(nrow(temp_df)), each = 4), ]
  lime_explanations_orig_norm <- explain(as.data.frame(temp_df[,c(1:7)]), 
                                           lime_explainer_orig_norm, n_labels = 1, 
                                           n_features = 7, n_permutations = 2000
                                           )
  print( plot_features(lime_explanations_orig_norm) )

  } 
dev.off()

pdf("./Results/lime_lu_explanations.pdf")
for( i in 1:nrow(origRF_norm_lu_pred_ims) ){
  temp_df <- data.frame(origRF_norm_lu_pred_ims[i,])
  temp_df <- temp_df[ rep(seq_len(nrow(temp_df)), each = 4), ]
  lime_explanations_orig_norm <- explain(as.data.frame(temp_df[,c(1:7)]), 
                                           lime_explainer_orig_norm, n_labels = 1, 
                                           n_features = 7, n_permutations = 2000
...
ive_rf <- individual_variable_effect(origRF, data = dataSetFull, predict_function = p_function,
            new_observation = sampleData, nsamples = 50)


ive_tree <- individual_variable_effect(origRF, data = dataSetFull, predict_function = p_function,
...
?shap()
shap1 <- shap(origRF, new_observation =  sampleData)
