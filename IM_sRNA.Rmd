...
**Side note:*** We used the word original to refer to the RF model created in the base article, or to refer to models that were created(trained) in the same way as the base random forest model. 
...
write.csv(dataSetTrain_scaled, "./DataSets/combinedData_scaled.csv", row.names = FALSE)
...
write.csv(dataSetTrain_norm, "./DataSets/combinedData_norm.csv", row.names = FALSE, col.names = TRUE)
...
# III) MODELS' PREDICTIONS COMPARISONS
...
### 10.3 SLT2 Data for the IMs
There are nearly 5000 potential test samples to analyze with any of the given local interpretability models. Since analyzing all of these values would go beyond our time restrictions, we narrowed down the analysis to only the false negative cases. Since it is expected that most potential sRNA samples will not be sRNA, looking for potential ways to increase the true positive rate of the model seemed like the best way to select which instances to analyze. 

```{r}
# Obtain all the instances that were classified as false negatives and save them to a data frame so that they can be later used by the local IM's for further analysis
slt2_predictions
slt2_im_data_fn <- slt2_predictions[slt2_predictions$Class == 1 & slt2_predictions$origPreds == 0,]
slt2_im_data_fp <- slt2_predictions[slt2_predictions$Class == 0 & slt2_predictions$origPreds == 1,]
slt2_im_data <- rbind(
  head( slt2_im_data_fn[ order( slt2_im_data_fn$OrigRF_P ), ], 10 ),
  tail( slt2_im_data_fp[ order( slt2_im_data_fp$OrigRF_P ), ], 10 )
)
slt2_im_data
...
### 11.3 LU Data for the IMs


```{r}
# Obtain all the instances that were classified as false negatives and false positives and save them to a data frame so that they can be later used by the local IM's for further analysis
lu_predictions
lu_im_data_fn <- lu_predictions[ lu_predictions$Class == 1 & lu_predictions$origPreds == 0, ]
lu_im_data_fp <- lu_predictions[ lu_predictions$Class == 0 & lu_predictions$origPreds == 1, ]
lu_im_data <- rbind(
  head( lu_im_data_fn[ order( lu_im_data_fn$OrigRF_P ), ], 10 ),
  tail( lu_im_data_fp[ order( lu_im_data_fp$OrigRF_P ), ], 10 )
)
lu_im_data
...
## 12. Other Potential Outliers - For Future Studies or Projects


### 12.2 SLT2 Data Outliers

```{r}

slt2data_max_SS      <- max(slt2data[,"SS"])
cbind(slt2data_max_SS, trainData_max_SS)
nrow(slt2data[slt2data$SS > trainData_max_SS,])

slt2data_min_SS      <- min(slt2data[,"SS"])
cbind(slt2data_min_SS, trainData_min_SS)
nrow(slt2data[slt2data$SS < trainData_min_SS,])

# ====================================================== #

slt2data_max_Pos     <- max(slt2data[,"Pos10wrtsRNAStart"])
cbind(slt2data_max_Pos, trainData_max_Pos)
nrow(slt2data[slt2data$Pos10wrtsRNAStart > trainData_max_Pos,])

slt2data_min_Pos     <- min(slt2data[,"Pos10wrtsRNAStart"])
cbind(slt2data_min_Pos, trainData_min_Pos) # Both Mins are the same

# ====================================================== #

slt2data_max_DisTerm <- max(slt2data[,"DistTerm"])
cbind(slt2data_max_DisTerm, trainData_max_DisTerm) # Both Max are the same

slt2data_min_DisTerm <- min(slt2data[,"DistTerm"])
cbind(slt2data_min_DisTerm, trainData_min_DisTerm)
nrow(slt2data[slt2data$DistTerm < trainData_min_DisTerm ,])

# ====================================================== #

slt2data_max_Dis     <- max(slt2data[,"Distance"])
cbind(slt2data_max_Dis, trainData_max_Dis) # Both Max are the same

slt2data_min_Dis     <- min(slt2data[,"Distance"])
cbind(slt2data_min_Dis, trainData_min_Dis)
nrow(slt2data[slt2data$Distance < trainData_min_Dis,])

# ====================================================== #
slt2data_max_DownDis <- max(slt2data[,"DownDistance"])
cbind(slt2data_max_DownDis, trainData_max_DownDis)
nrow(slt2data[slt2data$DownDistance > trainData_max_DownDis,])

slt2data_min_DownDis <- min(slt2data[,"DownDistance"])
cbind(slt2data_min_DownDis, trainData_min_DownDis) # Both Min are the same

# ====================================================== #
slt2_outliers <- slt2data[ slt2data$SS < trainData_min_SS | slt2data$SS > trainData_max_SS |  
                           slt2data$Pos10wrtsRNAStart < trainData_min_Pos | slt2data$Pos10wrtsRNAStart > trainData_max_Pos |
                           slt2data$DistTerm < trainData_min_DisTerm | slt2data$DistTerm > trainData_max_DisTerm |
                           slt2data$Distance < trainData_min_Dis | slt2data$Distance > trainData_max_Dis |
                           slt2data$DownDistance < trainData_min_DownDis | slt2data$DownDistance > trainData_max_DownDis 
                             ,]
...
### 12.2 LU Data Outliers

```{r}

ludata_max_SS      <- max(ludata[,"SS"])
cbind(ludata_max_SS, trainData_max_SS)
nrow(ludata[ludata$SS > trainData_max_SS,])

ludata_min_SS      <- min(ludata[,"SS"])
cbind(ludata_min_SS, trainData_min_SS)
nrow(ludata[ludata$SS < trainData_min_SS,])

# ====================================================== #

ludata_max_Pos     <- max(ludata[,"Pos10wrtsRNAStart"])
cbind(ludata_max_Pos, trainData_max_Pos)
nrow(ludata[ludata$Pos10wrtsRNAStart > trainData_max_Pos,])

ludata_min_Pos     <- min(ludata[,"Pos10wrtsRNAStart"])
cbind(ludata_min_Pos, trainData_min_Pos) # Both Mins are the same

# ====================================================== #

ludata_max_DisTerm <- max(ludata[,"DistTerm"])
cbind(ludata_max_DisTerm, trainData_max_DisTerm) # Both Max are the same

ludata_min_DisTerm <- min(ludata[,"DistTerm"])
cbind(ludata_min_DisTerm, trainData_min_DisTerm)
nrow(ludata[ludata$DistTerm < trainData_min_DisTerm ,]) # There are 454 samples outside this range!!!

# ====================================================== #

ludata_max_Dis     <- max(ludata[,"Distance"])
cbind(ludata_max_Dis, trainData_max_Dis) # Both Max are the same

ludata_min_Dis     <- min(ludata[,"Distance"])
cbind(ludata_min_Dis, trainData_min_Dis)
nrow(ludata[ludata$Distance < trainData_min_Dis ,])

# ====================================================== #
ludata_max_DownDis <- max(ludata[,"DownDistance"])
cbind(ludata_max_DownDis, trainData_max_DownDis)
nrow(ludata[ludata$DownDistance > trainData_max_DownDis ,])

ludata_min_DownDis <- min(ludata[,"DownDistance"])
cbind(ludata_min_DownDis, trainData_min_DownDis)
nrow(ludata[ludata$DownDistance < trainData_min_DownDis ,])

lu_outliers <- ludata[ ludata$SS < trainData_min_SS | ludata$SS > trainData_max_SS |  
                           ludata$Pos10wrtsRNAStart < trainData_min_Pos | ludata$Pos10wrtsRNAStart > trainData_max_Pos |
                           ludata$DistTerm < trainData_min_DisTerm | ludata$DistTerm > trainData_max_DisTerm | # trainData_min_DisTerm
                           ludata$Distance < trainData_min_Dis | ludata$Distance > trainData_max_Dis |
                           ludata$DownDistance < trainData_min_DownDis | ludata$DownDistance > trainData_max_DownDis 
                             ,]
...
Even though the following were not included in the feature interaction graph, we thought they it would be interesting to also have PDPs graphs for the following interactions:
  * DownDistance:DistTerm
  * Distance:DistTerm
  * Distance:DownDistance
  * sameStrand:sameDownStrand

**NOTE**: We used two different libraries (RFE and PDP) to generate the following plots. 

### 16.2 DownDistance vs. SS

Graph by RFE Library: 
...
Graph by PDP Library: 
![DownDistance vs. SS](./Results/pdp_DownDistance_vs_SS.png)
...
Graph by PDP Library: 
![Distance vs. SS](./Results/pdp_Distance_vs_SS.png)
...
Graph by PDP Library: 
![DownDistance vs. Pos10wrtsRNAStart](./Results/pdp_DownDistance_vs_Pos10wrtsRNAStart.png)  
...
Graph by PDP Library: 
![DistTerm vs. SS](./Results/pdp_DistTerm_vs_SS.png)
...
Graph by PDP Library: 
![Distance vs. Pos10wrtsRNAStart](./Results/pdp_Distance_vs_Pos10wrtsRNAStart.png) 
...
Graph by PDP Library:
![Distance vs. DownDistance](./Results/pdp_Distance_vs_DownDistance.png) 
...
Graph by PDP Library:
![DownDistance vs. DistTerm](./Results/pdp_DownDistance_vs_DistTerm.png)
...
Graph by PDP Library: 
![Distance vs. DistTerm](./Results/pdp_Distance_vs_DistTerm.png)
...
Graph by PDP Library: 
![sameStrand vs sameDownStrand](./Results/pdp_sameStrand_vs_sameDownStrand.png)
...
## 17. PDP plots for individual features
### 17.1 DownDistance PDP Plot
PDP plot using the PDP R library:
![PDP plot for ](./Results/pdp_DownDistance.png)


PDP plot using the H2O Library:
```{r}
h2o.partialPlot(object = rfh2o, data = trainData, cols="DownDistance")
```


### 17.2 Distance PDP Plot
PDP plot using the PDP R library:
![PDP plot for ](./Results/pdp_Distance.png)


PDP plot using the H2O Library:
```{r}
h2o.partialPlot(object = rfh2o, data = trainData, cols="Distance")
```

### 17.3 DistTerm PDP Plot
PDP plot using the PDP R library:
![PDP plot for ](./Results/pdp_DistTerm.png)


PDP plot using the H2O Library:
```{r}
h2o.partialPlot(object = rfh2o, data = trainData, cols="DistTerm")
```

### 17.4 SS PDP Plot
PDP plot using the PDP R library:
![PDP plot for ](./Results/pdp_SS.png)


PDP plot using the H2O Library:
```{r}
h2o.partialPlot(object = rfh2o, data = trainData, cols="SS")
```

### 17.5 Pos10wrtsRNAStart PDP Plot
PDP plot using the PDP R library:
![PDP plot for ](./Results/pdp_Pos10wrtsRNAStart.png)


PDP plot using the H2O Library:
```{r}
h2o.partialPlot(object = rfh2o, data = trainData, cols="Pos10wrtsRNAStart")
```
...
## 22. LIME in SLT2 and LU testing samples

Since we have proven that all of our models using the RandomForest library are similar enough, it is safe to conclude that the explanations from one model are also applicable to the other models. 
For this reason, we will use the normalized RF model as a proxy for the OrigRF model. 

```{r}

origRF_norm_slt2_pred_ims <- subset(slt2data_norm, rownames(slt2data_norm) %in% rownames(slt2_im_data) )
write.csv(origRF_norm_slt2_pred_ims, "./DataSets/slt2_data_to_analyze_normalized.csv", row.names = TRUE)
slt2_test_samples <- rbind(slt2_im_data[,c(1:8)], origRF_norm_slt2_pred_ims)
slt2_test_samples <- slt2_test_samples[order(row.names(slt2_test_samples)),]


origRF_norm_lu_pred_ims <- subset(ludata_norm, rownames(ludata_norm) %in% rownames(lu_im_data) )
write.csv(origRF_norm_lu_pred_ims, "./DataSets/lu_data_to_analyze_normalized.csv", row.names = TRUE)
lu_test_samples <- rbind(lu_im_data[,c(1:8)], origRF_norm_lu_pred_ims)
lu_test_samples <- lu_test_samples[order(row.names(lu_test_samples)),]

write.csv(slt2_test_samples, "./DataSets/slt2_data_to_analyze.csv", row.names = TRUE)
write.csv(lu_test_samples,   "./DataSets/lu_data_to_analyze.csv", row.names = TRUE)

all_test_samples <- rbind(slt2_test_samples,lu_test_samples)

write.csv(all_test_samples, "./DataSets/all_im_data_to_analyze_normalized_and_unaltered.csv", row.names = TRUE)

norm_test_samples <- rbind(origRF_norm_slt2_pred_ims, origRF_norm_lu_pred_ims)
...
#pdf("./Results/lime_explanations.pdf", paper = "legal", height =  11 ) # commented out so that a new PDF is not created
for( i in 1:nrow(norm_test_samples) ){
  temp_df <- data.frame(norm_test_samples[i,])
  temp_df <- temp_df[ rep(seq_len(nrow(temp_df)), each = 4), ]
  lime_explanations_orig_norm <- explain(as.data.frame(temp_df[,c(1:7)]), 
                                           lime_explainer_orig_norm, n_labels = 1, 
                                           n_features = 7, n_permutations = 2000
                                           )
  print( plot_features(lime_explanations_orig_norm, ncol = 1))
  } 
#dev.off()
...
## 23. LIME PRECONCLUSIONS 

A known problem with LIME is that explanations may be inconsistent from one instance to the next, even when the instances are very similar to each other. 
...
# VII) SHAP Values 
Unlike LIME, SHAP offers consistent results, regardless of how you input the data. It also curious to see that the feature contributions in the 3 H2O models behave similarly. 
...
SHAP_H2O2 <- h2o.predict_contributions(rfh2o_scaled, as.h2o(sampleData_scaled[,c(1:7)]))
head(SHAP_H2O2,10)

SHAP_H2O3 <- h2o.predict_contributions(rfh2o_norm, as.h2o(sampleData_norm[,c(1:7)]))
head(SHAP_H2O3,10)
```

According to SHAP, Distance is one of the least important features, yet, the lime explanations earlier would suggest that distance is one of the major contributors to the classification decision. 
